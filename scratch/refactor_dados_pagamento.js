...
// 1. Localizar o bloco antigo de "Dados do Pagamento"
const startMarker = '<div class="lanc-section-header">\\n                                    <div class="lanc-section-icon"><i class="fas fa-money-check-alt"></i></div>\\n                                    <span class="lanc-section-title">Dados do Pagamento</span>';
const endMarker = '<!-- ═══ OBSERVAÇÕES ═══ -->';

let sIndex = content.indexOf('<div class="lanc-section-icon"><i class="fas fa-money-check-alt"></i></div>');
if (sIndex === -1) {
    console.log("Falha ao localizar marcador inicial");
    process.exit(1);
}

// Retroceder até o começo da section
sIndex = content.lastIndexOf('<div class="lanc-section">', sIndex);

let eIndex = content.indexOf(endMarker);

if (sIndex !== -1 && eIndex !== -1) {
    console.log("Trecho encontrado! Substituindo...");
    
    const newHtml = `<!-- ═══ DADOS DO PAGAMENTO (MULTI-EMPENHO / MULTI-NF) ═══ -->
                                <div class="lanc-section">
                                  <div class="lanc-section-header">
                                    <div class="lanc-section-icon"><i class="fas fa-money-check-alt"></i></div>
                                    <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                                        <span class="lanc-section-title">Dados do Pagamento</span>
                                        <button type="button" class="lanc-btn-secondary" onclick="window.lancNovoBlocoEmpenho()" style="padding:0.4rem 0.8rem; border-color:#3b82f6; color:#1d4ed8; background:#eff6ff;"><i class="fas fa-folder-plus"></i> Adicionar Bloco de Empenho</button>
                                    </div>
                                  </div>
                                  <div class="lanc-section-divider"></div>

                                  <!-- Contêiner Reativo -->
                                  <div id="lanc-blocos-container">
    ...
    content = content.substring(0, sIndex) + newHtml + content.substring(eIndex);
} else {
    console.log("Trecho não encontrado. Provavelmente já foi alterado.");
}


// 2. Substituir Lógica de JS (Remover salvarMapa antigo e afins e injetar novas funcoes)
const jsFunctionsToRemove = [
    'window.lancAdicionarNF',
    'window.lancAtualizarTodosEmpenhosSelects',
    'window.lancAdicionarEmpenho',
    'window.lancRemoverEmpenho',
    'window.lancOnNovoEmpenhoChange',
...
let jsBlock = content.substring(scriptStart, scriptEnd);

jsFunctionsToRemove.forEach(fn => {
    // Basic regex to remove old definitions. This is fragile so we do it carefully.
    const fnRegex = new RegExp(fn.replace(".", "\\\\.") + '\\s*=\\s*(async\\s*)?function\\s*\\([\\s\\S]*?\\)\\s*{[\\s\\S]*?^\\s*};', 'gm');
    // since 'm' multiline matching is tricky with nested blocks, let's use indexOf and manual matching brackets.
});

// Since regex bracket matching is hard in JS, let's just append our block at the very end of the script tag to override existing function definitions! Because JS window objects can be safely overwritten.
...
console.log("Sucesso: Script reativo de Empenhos Dinâmicos aplicado!");
