...
async function main() {
    const { data: processes } = await supabase.from('processes').select('id, pp_ano, location').ilike('location', '%tesouraria%');
    console.log(`Found ${processes.length} processes`);
    processes.forEach(p => console.log(p.pp_ano, JSON.stringify(p.location)));
}
main();
