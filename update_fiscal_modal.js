...
const targetTabsAndFornecedorStart = content.indexOf('<div class="modal-body" style="padding: 1.5rem 2rem;">');
const targetFornecedorEndString = '</thead>\n                                        <tbody id="fornecedores-list">\n                                            <tr><td colspan="4" style="text-align: center; color: #999;">Nenhum fornecedor cadastrado</td></tr>\n                                        </tbody>\n                                    </table>\n                                </div>\n                            </div>';
const targetFornecedorEnd = content.indexOf(targetFornecedorEndString, targetTabsAndFornecedorStart);

if (targetTabsAndFornecedorStart === -1 || targetFornecedorEnd === -1) {
    console.error("Could not find targets!");
    process.exit(1); // will stop if not found
}
...
content = content.substring(0, targetTabsAndFornecedorStart) + replacement + content.substring(targetFornecedorEnd + targetFornecedorEndString.length);

// Also close the fiscal-main-content div at the very end of the modal body
const endTarget = content.indexOf('</div>\n                    </div>\n                </div>\n                `;\n                document.body.insertAdjacentHTML(');

if (endTarget !== -1) {
    const endReplacement = '</div>\n                            </div>\n                        </div>\n                    </div>\n                </div>\n                `;\n                document.body.insertAdjacentHTML(';
    content = content.replace('</div>\n                    </div>\n                </div>\n                `;\n                document.body.insertAdjacentHTML(', endReplacement);
}

// Next update the function atualizarTabelaFornecedores with the new styling for the buttons action
...
// Also update the padding for table cell rendering to align with the new header
